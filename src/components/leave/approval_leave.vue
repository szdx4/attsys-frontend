<template>
  <section>
    ...
    <el-dialog v-model="statusFormVisible" title="审核" :close-on-click-modal="false">
      <el-form ref="statusForm" :model="statusForm" label-width="80px" :rules="statusFormRules">
        <el-form-item label="工号" prop="id" align="left">
          <el-form-item :label="statusForm.user.id.toString()" auto-complete="off" />
        </el-form-item>
        <el-form-item label="姓名" prop="name" align="left">
          <el-form-item :label="statusForm.user.name" auto-complete="off" />
        </el-form-item>
        <el-form-item label="请假原因" prop="remark">
          <el-form-item :label="statusForm.remark" auto-complete="off" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    statusFormatter(row) {
      if (row.status === 'wait') { return '等待审核' } else if (row.status === 'pass') { return '已通过' } else if (row.status === 'reject') { return '不通过' } else return '未知状态'
    },
